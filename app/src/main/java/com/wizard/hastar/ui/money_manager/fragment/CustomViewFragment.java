...
public class CustomViewFragment extends Fragment {
        ...
        onDateSetListener = new DatePickerDialog.OnDateSetListener() {
            ...
            public void onDateSet(DatePickerDialog view, int year, int monthOfYear, int dayOfMonth) {
                if (isFrom) {
                    ...
                    if (to.before(from)) {
                        ToastUtil.displayShortToast(getActivity(), "起始时间有误");
                        ToastUtil.displayShortToast(getActivity(), "结束时间有误");
                    } else {
                        fromDate.setText(" ● " +
    ...
    private void select() {
        ...
        if (RecordManager.getInstance(MyApplication.getAppContext()).RECORDS == null
                || RecordManager.getInstance(MyApplication.getAppContext()).RECORDS.size() == 0) {
            return;
        }
        ...
        for (int i = start; i >= end; i--) {
            Record coCoinRecord = RecordManager.RECORDS.get(i);
